...
- status: open
- claimed_by: null
- scope: Autopilot's `swift test --skip ContactsResolverTests` run on 2026-05-06 20:39 EDT (commit `2cc9f6d` = `wip/autopilot-tests-2026-05-06-183430-menubar-waiting`, but reproduces on main too) executed 589 test cases successfully then HUNG indefinitely at `Tests/ReplyAITests/InboxViewModelTests.swift:1193` — `InboxViewModelIsSyncingTests.testIsSyncingFalseAfterSuccess`. The test itself is a simple 5-liner: create non-blocking `BlockingMockChannel`, await `vm.syncFromIMessage()`, assert `isSyncing == false`. xctest wrote no further output for 2+ minutes while the process stayed alive (CPU idle, not crashed — actor deadlock symptoms). The previous test in the suite, `testIsSyncingTrueWhileSyncFromIMessageIsRunning` (line ~1180), creates a BlockingMockChannel with `blocking = true`, blocks recentThreads via a continuation, asserts `isSyncing == true`, then calls `channel.resumeAll()` and `await syncTask.value`. **Suspected root cause**: a path through `testIsSyncingTrueWhile...` that doesn't hit `resumeAll()` (e.g. exception, early return, premature task cancellation), leaks the unresumed continuation, and the next test inherits a `@MainActor` deadlock when its sync task tries to acquire the same actor lane. Each test creates its own `vm` and channel, but the leaked continuation is bound to the actor, not to the channel instance. **Impact on autopilot**: this hang is a structural blocker for P1 wip-merges. The existing `--skip ContactsResolverTests` carve-out works around the headless-Contacts environmental issue; this is a NEW second-layer blocker that means even with that flag, autopilot's `swift test` gate cannot complete on warm cache.
- success_criteria:
