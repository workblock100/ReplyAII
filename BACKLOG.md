...
- candidate_fix_2026-05-09-0211 (autopilot, wip): branch `wip/autopilot-2026-05-09-0211-open-inbox-button-hit-test` (commit `1e7e364`) ships the `.contentShape(Capsule(style: .continuous))` fix candidate from the diagnostic above, with a full WHY comment cross-referencing the keyboard-shortcut-bypass behavior and the sibling `navButton` working-by-accident pattern. UI-sensitive so pushed to wip rather than main; awaits human visual review + smoke-launch verification before merge. Build verified clean (`swift build` succeeds, no new warnings beyond the pre-existing actor-context note in `SearchIndex.createSchema`). End-of-fire follow-up: re-checked the branch out and re-ran `swift test --skip ContactsResolverTests --skip InboxViewModelIsSyncingTests --skip InboxViewModelTests` — 1901 tests passing, 0 failures, 2 skipped (same shape as main). If the fix lands on main, criterion 1 of this REP flips to done and `claimed_by` can flip from `human` to the autopilot fire that authored the wip branch. **Confidence caveat (downgraded during action 9 review):** MenuBarContent's "Open inbox" button + ObDoneView's PrimaryButton both use the same `.background(Capsule().fill())` pattern and reportedly work, so the bug may instead be specific to the padding-derived sizing (no `.frame(height:)`); the wip is one hypothesis to test, not a confident fix. A second wip with `.frame(height: 30)` instead of `.contentShape` would give the human two candidates to visually compare — both being mechanical, isolated changes.
- success_criteria:
