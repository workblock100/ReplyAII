...
- `eb714ef` + `1b7d346` + `1de9d86` MenuBarContent Strings hoist — empty-state copy + footer CTAs + header brand glyph/name/waiting-chip suffix (7 hoisted strings, `MenuBarContentStringsTests` adds 9 XCTest pins). First view in the REP-UI-STR-HOIST-001 chain; rebased from a 9-day-old autopilot wip and ff-merged after passing the three-skip gate + 3-layer smoke launch (autopilot-2026-05-18-1628).
- `5d312cc` REP-222 voice profile WIRED through draft generation + auto-capture on successful send. `InboxViewModel.captureVoiceExample` appends each non-trivial send to `pref.voice.exampleMessages` (12-char floor, 20-entry FIFO, exact-duplicate de-dup); `MLXDraftService` reads the list and passes it to `PromptBuilder.build(voiceExamples:)` so on-device drafts use the user's recent prose as few-shot exemplars (autopilot-2026-05-18-1628, +15 tests via `InboxViewModelSendTests` + new `InboxViewModelVoiceCaptureTests`). Original REP-222 data layer landed on `autopilot-2026-05-03-2031`; today closes the wiring.
- `5e761c9` AccessibilityAPIReader (AX-based alt message source, injectable seams, 6 tests) + IMessageSender.retryDelay injectable (removes hardcoded 0.5s sleep, 1 new test + 3 updated) — REP-258+269 (landed on main; the original wip branch was reabsorbed when REP-501-style consolidation merged through)
- `7e932cf` per-channel Preferences keys + not(not(pred)) guard + concurrent add+remove tests — REP-231+208+220 (landed on main)
- `7c62474` SlackSocketClient: WebSocket wrapper for Socket Mode, injectable seams, 5 tests — REP-267 (landed on main; see Sources/ReplyAI/Channels/SlackSocketClient.swift + SlackSocketClientTests)
- `cfe50f8` Thread-list cache for cold-launch resilience — REP-278 (landed on main)
- `a819f59` ViewState enum + 4 transition tests — REP-247 (landed on main; SlackTokenStore consolidation in same commit)
...
- `80035e1` messageAgeOlderThan predicate, not/or/dismiss/concurrent/tone-distinctness tests (REP-097, REP-100, REP-106, REP-107, REP-112, REP-113, worker-2026-04-22-163000)
...
- **Slack / WhatsApp / Teams / Telegram / SMS**. `ChannelService` protocol exists. `SlackChannel` shipped (REP-233/234, commit `c001d7e`) and now sends/receives via `SlackHTTPClient` (REP-237/238, commit `e26e72a`); `LocalhostOAuthListener` (REP-230, `fbba843`) + `SlackOAuthFlow` (REP-272, `c975e51`) wire the auth flow; `SlackSocketClient` (REP-267, `7c62474`) handles Socket Mode receive. WhatsApp / Teams / Telegram / SMS remain stub channels (`Sources/ReplyAI/Channels/{WhatsApp,Teams,Telegram,SMS}Channel.swift`) that throw `authorizationDenied` from both `recentThreads` and `messages` until a real backend lands; the `ChannelStubKeychainContractTests` cluster pins their keychain service identifiers so a future "consistency" rename doesn't orphan an existing user's token.
...
- **Voice profile training**. `ob-voice` onboarding screen is still a UI mock and there is no LoRA pipeline. **Few-shot path is wired, though** (REP-222, main `5d312cc` 2026-05-18): `InboxViewModel.captureVoiceExample` appends each successful send to `pref.voice.exampleMessages` (20-entry FIFO, 12-char min, exact-duplicate de-dup), `MLXDraftService` reads that list and passes it to `PromptBuilder.build(voiceExamples:)` so on-device drafts use the user's recent prose as few-shot exemplars without any explicit training step. A Settings panel that reads + clears the profile lives on `wip/autopilot-2026-05-18-1640-voice-profile-settings-ui` awaiting human visual review. LoRA fine-tuning would replace the few-shot path with on-device weights, but the few-shot path alone is usable as a v1.
...
- **MLX runtime exit bug — RESOLVED 2026-05-19.** Per REP-ALERT-260504-1650 (status `done`, `done_on: 540d760`), the bundled `.app` used to exit ~1s after launch when `pref.model.useMLX = true` because dynamic library loading of `MLXLLM` / `MLXHuggingFace` / `Tokenizers` on the launch path triggered a clean exit. The structural fix shipped via the REP-501→REP-505 SPM target split — MLX symbols now live in the `ReplyAIMLX` target and only load when the user opts in. Smoke-launch with `pref.model.useMLX = true` on commit 540d760 confirmed PID alive after 30s. The historical `defaults write co.replyai.mac pref.model.useMLX -bool false` workaround is no longer needed for correctness but stays as the shipped default until the MLX user-toggle in Settings has been polished (see REP-AUDIT-260505 follow-ups).
...
- **Date-delta tests with hour-granularity offsets are clock-sensitive — use Calendar instead.** A test that builds a "clearly yesterday" or "clearly N days ago" timestamp via `Date().addingTimeInterval(-25 * 3_600)` (or any sub-day delta) is fragile: at run-times near midnight the absolute-seconds offset crosses two midnights and lands in the wrong calendar bucket. Concrete failure surfaced 2026-05-09 at fire start time 00:11 EDT — `IMessageChannelFormatTimeTests.testFormatRelativeUsesHoistedYesterdayLabel` used `Date() - (86_400 + 3_600)` ("25h ago"), which at 00:11 was actually 23:11 two days ago, so `isDateInYesterday` correctly rejected it and the formatter returned `"Thu"` instead of `"Yesterday"`. **Fix pattern**: build the target date via Calendar — `Calendar.current.date(byAdding: .day, value: -1, to: Calendar.current.startOfDay(for: Date()))!.addingTimeInterval(12 * 3_600)` for yesterday-noon. The sibling `testFormatRelativeYesterdayBucketReturnsYesterdayLiteral` already used this pattern and stayed green; the deflake repaired the older sibling to match. When writing any time-bucket test, ask: would this still pass if it ran at 00:01 local? At 23:59? At a DST transition? If any of those is "no", switch to Calendar arithmetic against `startOfDay(for:)`. Same caveat applies to multi-day deltas (`-5 * 86_400`) — they're clock-sensitive at DST boundaries even if not at midnight.
...
- ~~Thread-select bar: animate the `Rectangle().fill(isSelected ? accent : .clear)` with `withAnimation(Theme.Motion.std)` and `matchedGeometryEffect` across rows.~~ Resolved (REP-082, shipped on main `a004936`, 2026-05-07): `ThreadListView` declares `@Namespace selectionNamespace` and forwards it to `ThreadRow`, whose accent bar uses `matchedGeometryEffect(id: "thread-selection-bar", in: ns)` when `!reduceMotion && isSelected`. Default-arg `selectionNamespace: Namespace.ID? = nil` on `ThreadRow` keeps gallery call sites compiling.
...
- ~~Reduced motion: read `@Environment(\.accessibilityReduceMotion)`, skip crossfades in `ComposerView.editableDraft` and tone-pill animations.~~ Resolved (REP-083, shipped on main `a004936`, 2026-05-07): 13 `withAnimation` sites across `PillToggle`, `TonePills`, `ThreadListView`, `InboxScreen`, and `SidebarView` gate on `@Environment(\.accessibilityReduceMotion)`. `ComposerView.editableDraft` has no animation modifiers in current main (refactored out), so nothing was left to gate there.
