...
- **`swift test` full-suite hangs intermittently on Contacts XPC even with `--skip ContactsResolverTests`.** Symptoms: xctest CPU drops to 0%, total CPU time stays under 10s, system log spams `CoreData: error: Failed to create NSXPCConnection` against `AddressBook-v22.abcddb`. The XCTSkipIf in `ContactsResolverTests.setUpWithError()` no-ops the suite when Contacts auth isn't granted, but the framework is loaded eagerly when the test bundle initializes — that load itself can hang on the system AddressBook daemon for reasons that are not deterministic. Observed 2026-05-05-0411 fire: full suite passed in 14.5s at the start of the fire, then hung indefinitely on subsequent runs in the same fire. **2026-05-05-0611 update — hang reproducibly stops at the same spot.** Two separate runs in this fire (180s and 240s timeouts) both completed `InboxViewModelBulkFilterTests` and then sat with the test bundle alive but no further `Test Case` lines emitted; nothing after the alphabetically-next suite (`InboxViewModelChatGUIDDeduplicationTests`) ever ran. The hang is no longer "intermittent on this runner" — it's deterministic right at that boundary, suggesting an instance-level lazy load of the Contacts framework triggered by something in InboxViewModel's setup path that runs once during the first ChatGUID test's class init. Workers should not waste cycles re-running with longer timeouts. **Workaround**: when full suite hangs, run targeted `--filter <SuiteName>` to verify your change in isolation, then leave the branch on `wip/` rather than merging unverified to main; a future fire (or a `tccutil reset Contacts` on the runner) usually clears the hang.
