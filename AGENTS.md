...
Tests/ReplyAITests/                ~1938 tests as of 2026-05-09-0811 (1901 pass under the autopilot's three-skip workaround — `--skip ContactsResolverTests --skip InboxViewModelIsSyncingTests --skip InboxViewModelTests` — in ~18.6s warm; the other ~37 live in those three skipped suites, see the "`swift test` full-suite hangs intermittently on Contacts XPC" gotcha below). Plus 2 always-skipped in headless: `GlobalHotkeyContractTests` AppKit-touching cases gated behind `RUN_APPKIT_TOUCHING_TESTS=1`; opt-in to exercise locally.
```
